...
const MyCourses = () => {
          ...
          const ratingPromises = uiCourses.map(async (c) => {
            ...
            try {
              ...
              const res = await fetch(
                `${API_BASE}/api/course/${c.id}/rating`, //my-rating
                {
                  method: "GET",
                  headers: rHeaders,
                  credentials: "include",
  ...
  //to submit the rating to server
  // Helper: optimistic submit rating to server(similar to the courses page)
  ...
  if (!courses || courses.length === 0) {
    return (
      <div className={myCoursesStyles.pageContainer}>
        <div className={myCoursesStyles.mainContainer}>
          <h1 className={myCoursesStyles.header}> </h1>
        ...
        <div className={myCoursesStyles.grid}>
          {courses.map((course, index) => (
            <div
              ...
              style={{
                animationDelays: `${index * 100}ms`,
                animation: `faseInUp 0.6s ease-out ${index * 100}ms both`,
              }}
              role="button"
              tabIndex={0}
              onKeyDown={(e) => {
